...
class CustomDataset(_BaseDataset):
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        for t in range(num_timesteps):
            ...
            if time_key in read_data.keys():
                ...
                time_data = np.asarray(read_data[time_key], dtype=float)  # np.float
            ...
            if is_gt:
                ...
                if time_key in ignore_data.keys():
                    time_ignore = np.asarray(ignore_data[time_key], dtype=float)  # np.float
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            gt_class_mask = gt_class_mask.astype(bool)  # np.bool
            ...
            tracker_class_mask = tracker_class_mask.astype(bool)  # np.bool
            ...
            to_remove_matched = np.array([], int)  # np.int
        ...
        if len(unique_gt_ids) > 0:
            ...
            for t in range(raw_data['num_timesteps']):
                if len(data['gt_ids'][t]) > 0:
                    data['gt_ids'][t] = gt_id_map[data['gt_ids'][t]].astype(int)  # np.int
        if len(unique_tracker_ids) > 0:
            ...
            for t in range(raw_data['num_timesteps']):
                if len(data['tracker_ids'][t]) > 0:
                    data['tracker_ids'][t] = tracker_id_map[data['tracker_ids'][t]].astype(int)  # np.int
